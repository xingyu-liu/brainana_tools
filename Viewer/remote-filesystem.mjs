...
export class SshFilesystem {
  constructor({ target, controlSocket, root, cacheRoot }) {
    if (!target || !controlSocket || !root || !cacheRoot) throw new Error('Incomplete SSH filesystem configuration')
    this.target = target
  ...
  cleanRelative(raw = '') {
    ...
    if (parts.some((part) => part === '.' || part === '..' || part.includes('\0'))) throw new Error('Invalid remote path')
    return parts.join('/')
  }
  ...
  relative(absolute) {
    ...
    if (!normalized.startsWith(`${this.root}/`)) throw new Error('Remote path is outside configured root')
    return normalized.slice(this.root.length + 1)
  }

  run(script, { input, encoding = 'utf8', maxBuffer = 128 * 1024 * 1024 } = {}) {
    const result = spawnSync(SSH_BIN, ['-S', this.controlSocket, '-o', 'BatchMode=yes', this.target, `sh -c ${quote(script)}`], {
      input,
      encoding,
      maxBuffer,
    })
    if (result.error) throw result.error
    if (result.status !== 0) throw new Error((result.stderr || `Remote command failed with status ${result.status}`).toString().trim())
    return result.stdout
  }

  health() {
    return String(this.run('printf brainana-ssh-ok')).trim() === 'brainana-ssh-ok'
  }

  stat(relative) {
    const abs = this.absolute(relative)
    const text = String(this.run(`stat -L -c '%F\t%s\t%Y' -- ${quote(abs)}`)).trim()
    const [kind, size, mtime] = text.split('\t')
    return { isFile: kind === 'regular file', isDirectory: kind === 'directory', size: Number(size), mtimeMs: Number(mtime) * 1000 }
  }

  exists(relative, kind = null) {
    const abs = this.absolute(relative)
    const test = kind === 'file' ? '-f' : kind === 'directory' ? '-d' : '-e'
    const result = spawnSync(SSH_BIN, ['-S', this.controlSocket, '-o', 'BatchMode=yes', this.target, `sh -c ${quote(`test ${test} ${quote(abs)}`)}`])
    return result.status === 0
  }

  list(relative = '', { recursive = false, maxDepth = 1, includeHidden = false } = {}) {
    const abs = this.absolute(relative)
    ...
    return output.split('\n').filter(Boolean).map((line) => {
      ...
      return {
        type: type === 'd' ? 'directory' : type === 'f' || type === 'l' ? 'file' : 'other',
  ...
  cachePath(relative) {
    const clean = this.cleanRelative(relative)
    const digest = crypto.createHash('sha256').update(`${this.target}\0${this.root}\0${clean}`).digest('hex').slice(0, 20)
  ...
  ensureCached(relative) {
    ...
    try {
      ...
      valid = fs.existsSync(dest) && meta.size === info.size && meta.mtimeMs === info.mtimeMs && fs.statSync(dest).size === info.size
    } catch {}
    ...
    let result
    try {
      result = spawnSync(SSH_BIN, ['-S', this.controlSocket, '-o', 'BatchMode=yes', this.target, `cat -- ${quote(this.absolute(clean))}`], {
    ...
    if (result.error) { fs.rmSync(temp, { force: true }); throw result.error }
    if (result.status !== 0) { fs.rmSync(temp, { force: true }); throw new Error((result.stderr || 'Unable to download remote file').toString().trim()) }
    if (fs.statSync(temp).size !== info.size) { fs.rmSync(temp, { force: true }); throw new Error('Remote file transfer was incomplete') }
    fs.renameSync(temp, dest)
    fs.writeFileSync(metaPath, JSON.stringify({ size: info.size, mtimeMs: info.mtimeMs, relative: clean }))
    return dest
  }

  createReadStream(relative) {
    const clean = this.cleanRelative(relative)
    return spawn(SSH_BIN, ['-S', this.controlSocket, '-o', 'BatchMode=yes', this.target, `cat -- ${quote(this.absolute(clean))}`], { stdio: ['ignore', 'pipe', 'pipe'] })
  }
  ...
  async writeStream(relative, readable, { overwrite = false } = {}) {
    ...
    const child = spawn(SSH_BIN, ['-S', this.controlSocket, '-o', 'BatchMode=yes', this.target, `sh -c ${quote(script)}`], { stdio: ['pipe', 'pipe', 'pipe'] })
    ...
    if (status !== 0) throw new Error(stderr.trim() || `Remote write failed with status ${status}`)
